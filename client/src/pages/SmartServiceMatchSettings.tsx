...
import { ArrowLeft, ChevronDown, ChevronRight, ShieldCheck, Sparkles } from "lucide-react";
        ...
        <main className="px-4 pt-5 space-y-4 max-w-lg mx-auto">
          ...
          <section className="rounded-2xl bg-white border border-border overflow-hidden">
            <div className="px-4 py-4 flex items-center justify-between gap-4">
              ...
              <button aria-label="Enable Smart Service Match" aria-pressed={settings?.globalEnabled} onClick={() => update.mutate({ globalEnabled: !settings?.globalEnabled })} disabled={update.isPending} className={cn("relative h-7 w-12 rounded-full p-1 transition-colors", settings?.globalEnabled ? "bg-primary" : "bg-muted")}><span className={cn("block h-5 w-5 rounded-full bg-white shadow-sm transition-transform", settings?.globalEnabled ? "translate-x-5" : "translate-x-0")} /></button>
            </div>
          </section>
          ...
          <section className="rounded-2xl bg-white border border-border overflow-hidden">
            ...
            {showServiceCustomization && <div className="border-t border-border">
              ...
              <button aria-label={`Enable Smart Match for ${service.customName || service.category}`} aria-pressed={service.smartMatchEnabled && settings?.globalEnabled} onClick={() => update.mutate({ serviceId: service.id, serviceEnabled: !service.smartMatchEnabled })} disabled={update.isPending || !settings?.globalEnabled} className={cn("relative h-6 w-11 rounded-full p-1 transition-colors", service.smartMatchEnabled && settings?.globalEnabled ? "bg-primary" : "bg-muted")}><span className={cn("block h-4 w-4 rounded-full bg-white shadow-sm transition-transform", service.smartMatchEnabled && settings?.globalEnabled ? "translate-x-5" : "translate-x-0")} /></button>
            </div>)}
